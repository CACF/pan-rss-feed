...
class SportsnetScraper:
    ...
    def clean_content(html):
        ...
        try:
            ...
            for tag in soup(["script", "style", "aside", "iframe", "noscript", "button"]):
                tag.decompose()
        ...
        try:
            ...
            for section_url in cls.SECTION_URLS:
                ...
                try:
                    ...
                    if not data:
                        logger.warning(f"No __NEXT_DATA__ found on {section_url}, skipping")
                        continue

                    try:
                        top_level = data["props"]["pageProps"]["page"]["data"]
                    except (KeyError, TypeError):
                        logger.warning(f"Unexpected __NEXT_DATA__ structure on {section_url}")
                        continue

                    section_links = []
                    cls._walk_collections(top_level, section_links, cls.ARTICLE_POST_TYPES)

                    logger.info(f"Found {len(section_links)} article links on {section_url}")
                    ...
                    for href in section_links:
                        ...
                        if href and 'sportsnet.ca' in href:
                            if not any(x in href for x in ['/live/', '/video/', '/radio/', '/news/', '/tag/', '/search']):
    ...
    def fetch_article(cls, url):
        ...
        try:
            ...
            if not title_elem:
                title_elem = soup.select_one("h1.detailHeadline, h1.article-title, h1.title, h1")
            ...
            cutoff = datetime.now(timezone.utc) - timedelta(days=cls.MAX_ARTICLE_AGE_DAYS)
            if pub_date < cutoff:
                logger.debug(f"Skipping stale article ({pub_date.isoformat()}) for {url}")
                return None
            
            author_elem = soup.select_one(".micro_author > span > a:first-of-type")
            if not author_elem:
                author_elem = soup.select_one(".bylineDetails .authorText a, .author a, .byline a")
            author = author_elem.get_text(strip=True) if author_elem else "Sportsnet Staff"
            ...
            if len(content) < 200:
                logger.warning(f"Content too short ({len(content)} chars) for {url}, skipping")
                return None
    ...
    def run_pipeline(input_data=None):
        ...
        try:
            ...
            logger.info(f"Found {len(article_links)} article links. Fetching articles...")
            ...
            logger.info(f"Successfully fetched {len(all_articles)} unique articles. Failed: {failed_count}")
            
            if all_articles:
                sample = all_articles[0]
                logger.info(f"Sample - Title: {sample['title'][:50]}..., Genre: {sample['genre']}, Content length: {len(sample['content'])}")
