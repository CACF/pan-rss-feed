...
class CBCSportsScraper:
    ...
    def fetch_genre_pages(cls):
        ...
        try:
            ...
            resp = requests.get(cls.BASE_URL, timeout=30, headers=get_random_headers(cls.HEADERS))
            try:
            ...
            if not found_links:
                logger.warning("No genre links found with specific selectors, trying to find all sports links...")
                all_links = soup.find_all("a", href=True)
                for link in all_links:
                    href = link.get("href", "")
                    # Look for sports-related links
                    if "/sports/" in href and not href.startswith("#") and not href.startswith("javascript:"):
                        # Filter out common non-genre pages
                        if not any(x in href.lower() for x in ["/live/", "/video/", "/news/", "/radio/"]):
            ...
            for link_elem in found_links:
                ...
                if href:
                    ...
                    if href not in genre_urls and href != cls.BASE_URL and href != "https://www.cbc.ca/sports/":
    ...
    def fetch_article_links_from_genre(cls, genre_url):
        ...
        try:
            ...
            resp = requests.get(genre_url, timeout=30, headers=get_random_headers(cls.HEADERS))
            try:
                resp.raise_for_status()
                payload = resp.content
            finally:
            ...
            for selector in selectors:
                elements = soup.select(selector)
                if elements:
                    logger.info(f"Found {len(elements)} article links with selector: {selector}")
                    found_links.extend(elements)
            ...
            for a in found_links:
                ...
                if href:
                    ...
                    if href and "/sports/" in href and not any(x in href.lower() for x in ["/live/", "/video/"]):
                        links.append(href)
                        logger.debug(f"Found article link: {href}")
            
            # If no links found, try to find all article links on the page
            if not links:
                logger.warning(f"No links found with specific selectors for {genre_url}, trying to find all article links...")
                all_links = soup.find_all("a", href=True)
                for link in all_links:
                    href = link.get("href", "")
                    if "/sports/" in href and not href.startswith("#"):
                        # Check if it's likely an article link
                        if not any(x in href.lower() for x in ["/live/", "/video/", "/radio/", "/news/"]):
                            if href.startswith("/"):
                                href = "https://www.cbc.ca" + href
                            elif not href.startswith("http"):
                                href = urljoin(genre_url, href)
    ...
    def fetch_article(cls, url):
        ...
        try:
            ...
            title_elem = soup.select_one("h1.detailHeadline")
            ...
            if story_div:
                ...
                for p in paragraphs:
                    ...
                    if text and len(text) > 10 and not text.lower().startswith("copyright"):
                        content_paragraphs.append(text)
            
            # Fallback: try other common content containers
            if not content_paragraphs:
                logger.debug(f"No content found with primary selector for {url}, trying fallbacks...")
                ...
                if not content_paragraphs:
                    article_content = soup.select("article p, .content p, .main-content p")
            ...
            if len(content) < 50:
                logger.warning(f"Content too short ({len(content)} chars) for {url}, might not be an article")
            ...
            logger.debug(f"Successfully parsed article: {title[:50]}... ({len(content)} chars)")
            return article
    ...
    def fetch_all_articles(cls):
        ...
        try:
            ...
            logger.info(f"Found {len(genre_pages)} genre pages. Fetching articles from each...")
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
                futures = {
                    executor.submit(cls.fetch_article_links_from_genre, genre_url): genre_url
    ...
    def run_pipeline(input_data=None):
        ...
        try:
            ...
            logger.info(f"Found {len(article_links)} article links. Starting to fetch articles...")
            ...
            if all_articles:
                ...
                logger.info(f"Sample article - Title: {sample['title'][:50]}..., Content length: {len(sample['content'])}")
            else:
            ...
            logger.info(f"Inserted {result.get('inserted_count', 0)} articles into database")
            return result
