...
class HockeyPaperScraper:
    ...
    def fetch_article_links(cls):
        ...
        try:
            ...
            resp = cls.SCRAPER.get(cls.LISTING_URL, timeout=30, headers=get_random_headers(cls.HEADERS))
            try:
            ...
            headings = soup.select(".tdb_module_loop.td_module_wrap h3.entry-title.td-module-title a")
            links = []
            for a in headings:
                href = a.get("href")
                if href:
    ...
    def fetch_full_description(soup):
        ...
        Note: .tdb-block-inner.td-fix-index is not unique on the page (the
        author bio block reuses the same classes), so we select paragraphs
        directly across the whole page rather than scoping to one container
        first — scoping to a single select_one() container can silently
        grab the author block instead of the article body.
        """
        paragraphs = soup.select("div.tdb-block-inner.td-fix-index p.wp-block-paragraph")
    ...
    def fetch_article(cls, url):
        ...
        try:
            resp = cls.SCRAPER.get(url, timeout=30, headers=get_random_headers(cls.HEADERS))
            try:
                resp.raise_for_status()
                payload = resp.content
            finally:
            ...
            time_elem = soup.select_one("div.tdb-block-inner > time.entry-date")
            date_str = (
