...
class PotoharRSSPipeline:
    ...
    def process_item(
        ...
        if it should be skipped). Contains the network-bound work (Google News
        link resolution + full article fetch), so it's safe to run from a
        worker thread: it only touches its own local variables and read-only
        class data, never shared state. Logic and ordering are unchanged from
        the original sequential loop — including the pre-existing quirk where
        the domain skip-check runs after the full article fetch rather than
        before it.
        """
        try:
            ...
            if not PotoharRSSPipeline.is_potohar_related(title):
                logger.info(f"Not Potohar Keywords Related, skipping: '{title}'")
                return None
    ...
    def fetch_rss_feed(
        ...
        try:
            ...
            # I/O-bound per item (Google News decode + full article fetch),
            # so a thread pool gives a real speedup here — threads release
            # the GIL while waiting on network calls.
