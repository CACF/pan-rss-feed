...
from pipeline_maps.karachi_pipeline_map import SCRAPERS

logger = logging.getLogger(__name__)


class KarachiPipeline:
    """Pipeline for managing and running all Karachi scrapers from karachi_pipeline_map.py."""

    CATEGORY = "Karachi"

    @classmethod
    def run_pipeline(cls, input_data=None, table_name=None, on_scraper_start=None, on_scraper_finish=None, max_workers=10):
        """Execute all Karachi scrapers registered in karachi_pipeline_map.py concurrently."""
        target_table = table_name or KARACHI_TABLE
        results = []
        scrapers_to_run = SCRAPERS

        if not scrapers_to_run:
            logger.info("No Karachi scrapers enabled in pipeline_maps/karachi_pipeline_map.py")
            return results
