...
from pipeline_maps.sports_pipeline_map import SCRAPERS

logger = logging.getLogger(__name__)


class SportsPipeline:
    """Pipeline for managing and running all Sports scrapers from sports_pipeline_map.py."""

    CATEGORY = "Sports"

    @classmethod
    def run_pipeline(cls, input_data=None, table_name=None, on_scraper_start=None, on_scraper_finish=None, max_workers=10):
        """Execute all sports scrapers registered in sports_pipeline_map.py concurrently."""
        target_table = table_name or SPORTS_TABLE
        results = []
        scrapers_to_run = SCRAPERS

        if not scrapers_to_run:
            logger.info("No sports scrapers enabled in pipeline_maps/sports_pipeline_map.py")
            return results
