...
class FourFourTwoRSSPipeline:
    ...
    def fetch_rss_feed(feed_url):
        ...
        try:
            ...
            feed_build_date = (
                FourFourTwoRSSPipeline.parse_date(soup.find("lastBuildDate").get_text())
            ...
            for item in items:
                try:
                    ...
                    pub_date = (
                        FourFourTwoRSSPipeline.parse_date(item.pubDate.get_text())
                        if item.pubDate
                        else datetime.now(timezone.utc)
                    )
                    ...
                    tags = [
                        tag.get_text(strip=True) for tag in item.find_all("category")
                    ]
                    ...
                    logger.warning(f"Failed to process FourFourTwo article: {e}")
                    continue

            logger.info(f"Parsed {len(articles)} FourFourTwo articles from feed.")
    ...
    def run_pipeline(input_data=None, table_name=None):
        try:
            ...
            for feed in FourFourTwoRSSPipeline.RSS_FEEDS:
                all_articles.extend(FourFourTwoRSSPipeline.fetch_rss_feed(feed))
