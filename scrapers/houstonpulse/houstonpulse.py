...
from .keywords import PAKISTAN_KEYWORDS
from .feeds import GOOGLE_NEWS_FEEDS, HOUSTON_LOCAL_FEEDS
...
class HoustonPulseRSSPipeline:
    ...
    def is_pakistan_related(text):
        lower = text.lower()
        return any(kw in lower for kw in PAKISTAN_KEYWORDS)
    ...
    def process_item(
        ...
        **kwargs
        ...
        try:
            ...
            source = resolve_source(item, HoustonPulseRSSPipeline.SOURCE)
        ...
        try:
            ...
            for feed_url in GOOGLE_NEWS_FEEDS:
                all_articles.extend(
                    fetch_rss_feed(
                        feed_url=feed_url,
                        process_item_func=HoustonPulseRSSPipeline.process_item,
            ...
            return SupabaseClient.insert_system_articles(
                "houstonpulse", all_articles, table_name=target_table
            )
