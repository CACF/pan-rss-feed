...
from pipeline_maps.business_pipeline_map import SCRAPERS

logger = logging.getLogger(__name__)


class BusinessPipeline:
    """Pipeline for managing and running all Business scrapers from business_pipeline_map.py."""

    CATEGORY = "Business"

    @classmethod
    def run_pipeline(cls, input_data=None, table_name=None, on_scraper_start=None, on_scraper_finish=None, max_workers=10):
        """Execute all business scrapers registered in business_pipeline_map.py concurrently."""
        target_table = table_name or BUSINESS_TABLE
        results = []
        scrapers_to_run = SCRAPERS

        if not scrapers_to_run:
            logger.info("No business scrapers enabled in pipeline_maps/business_pipeline_map.py")
            return results
