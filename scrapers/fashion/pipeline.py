...
from pipeline_maps.fashion_pipeline_map import SCRAPERS

logger = logging.getLogger(__name__)


class FashionPipeline:
    """Pipeline for managing and running all Fashion scrapers from fashion_pipeline_map.py."""

    CATEGORY = "Fashion"

    @classmethod
    def run_pipeline(cls, input_data=None, table_name=None, on_scraper_start=None, on_scraper_finish=None, max_workers=10):
        """Execute all fashion scrapers registered in fashion_pipeline_map.py concurrently."""
        target_table = table_name or FASHION_TABLE
        results = []
        scrapers_to_run = SCRAPERS

        if not scrapers_to_run:
            logger.info("No fashion scrapers enabled in pipeline_maps/fashion_pipeline_map.py")
            return results
